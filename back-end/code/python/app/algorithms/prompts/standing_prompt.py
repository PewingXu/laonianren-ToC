...
{{
  ...
  "overview": "用于报告顶部评分卡（页眉）。口语化、面向老人/家属，结合 score_context 的评分等级，综合性地说清楚总体站立平衡稳不稳、要不要紧、下一步大方向往哪努力。**撇开具体数据——不引用小项得分、不报数值、不逐条点名小项（足弓/分区等细节留到页脚）**，不展开训练处方，约 110-150 字。",
  "arch_analysis": "专业分析足弓结构。必须引用并解释 score_context 中‘足弓/足型压力特征’的得分，说明高弓足/正常/扁平足对负荷分布和步态稳定性的意义。约 130-180 字。",
  "pressure_balance_analysis": "专业分析负荷分布。必须引用并解释 score_context 中‘左右负荷偏移（核心）’与‘足底压力分区合理性’两项的得分，指出偏侧负重或前后负荷偏移的功能意义。约 130-180 字。",
  "stability_analysis": "专业分析姿势控制稳定性。必须引用并解释 score_context 中‘四阶段平衡等级（核心）’‘COP稳态水平（核心）’‘COP轨迹形态’‘AP/ML稳定性’的得分，结合其计算方式说明站立稳定性与跌倒风险倾向。约 130-180 字。",
