...
1. 完整周期数 < 3（连完整 3 次坐站都没做完，不能视为完成测试）
2. 总时长 < 5 秒（5 次起坐物理上不可能少于 5 秒，少于此值说明根本没认真做）
...
- data_quality.suggestion = "请重新规范完成 5 次完整起坐动作（完整坐下→完整站起为 1 次）后再做评估"
...
- 12~15 秒：稍慢，需要关注
- 15~20 秒：偏慢，需要锻炼
- >20 秒：明显慢，坐站转移能力明显下降，要重视并建议进一步评估下肢肌力与衰弱风险
...
{{
  ...
  "performance_analysis": "专业分析下肢转移动作能力。必须引用并解释 score_context 中‘5次起坐总时长（核心）’与‘周期稳定性’两项的得分（得了几分、为什么），结合其计算方式说明节律稳定性与疲劳趋势。约 130-180 字。",
