...
{{
  ...
  "overview": "用于报告顶部评分卡（页眉）。口语化、面向老人/家属，结合 score_context 的评分等级，综合性地说清楚总体握力水平怎么样、要不要紧、下一步大方向往哪努力。**撇开具体数据——不引用小项得分、不报数值、不逐条点名小项**，不展开训练处方，约 110-150 字。",
  "left_hand_analysis": "只分析左手。结合 score_context 中‘最大握力（核心）’‘手指贡献均衡性’‘抓握稳定性’的得分（解释为什么得这个分），围绕峰值握力、手指力分布、姿态稳定性专业判读；无数据时说明缺失。约 130-180 字。",
  "right_hand_analysis": "只分析右手。结合 score_context 中‘最大握力（核心）’‘手指贡献均衡性’‘抓握稳定性’的得分（解释为什么得这个分），围绕峰值握力、手指力分布、姿态稳定性专业判读；无数据时说明缺失。约 130-180 字。",
  "bilateral_comparison": "必须引用并解释 score_context 中‘左右手差异’的得分，分析双侧握力差、优势侧、对称性及功能意义；只有单侧数据时说明无法完成双侧比较。约 120-160 字。",
