...
COMMON_ASSESSMENT_USER_NOTE = (
    ...
    "- `overview` 建议 110~150 字，给出整体判断、主要风险方向和下一步观察重点。\n"
    "- 所有分析段（如 `*_analysis`、`*_comparison`）每段必须写到 115~150 字，可从多个角度展开，包括趋势、对称性、稳定性、波动、风险倾向、站姿/步态/发力特征推断等。字数不足 115 字的段落需要补充分析角度。\n"
    "- `clinical_suggestion` 建议 3~4 条；每条约 50~75 字，总体保持中等篇幅、可执行、具体可操作。\n"
