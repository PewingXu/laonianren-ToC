...
import { analyzeGaitCSV } from '../../lib/gripPythonApi';
...
const PAD_LABELS = ['垫子一', '垫子二', '垫子三', '垫子四'];

function GaitConnectDialog({ open, onClose, onConnected }) {
  const [statuses, setStatuses] = useState(['idle', 'idle', 'idle', 'idle']);
  const [errors, setErrors] = useState(['', '', '', '']);

  const setOneStatus = (idx, s) => setStatuses(prev => { const n = [...prev]; n[idx] = s; return n; });
  const setOneError = (idx, e) => setErrors(prev => { const n = [...prev]; n[idx] = e; return n; });

  const connectPad = async (idx) => {
    const key = SENSOR_KEYS[idx];
    const service = footpadServices[key];
    setOneStatus(idx, 'connecting');
    setOneError(idx, '');
    try {
      const ok = await service.connect();
      if (ok) {
        setOneStatus(idx, 'connected');
      } else {
        setOneStatus(idx, 'error');
        setOneError(idx, '用户取消或连接失败');
      }
    } catch (e) {
      setOneStatus(idx, 'error');
      setOneError(idx, e.message || '连接失败');
    }
  };
  ...
  const handleConfirm = () => {
    if (anyConnected) onConnected(statuses);
  };

  const handleCancel = async () => {
    for (let i = 0; i < 4; i++) {
      if (statuses[i] === 'connected') {
        try { await footpadServices[SENSOR_KEYS[i]].disconnect(); } catch (e) {}
      }
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center zeiss-overlay animate-fadeIn">
      <div className="zeiss-dialog p-6 min-w-[440px] animate-scaleIn">
        <h3 className="text-lg font-bold mb-1" style={{ color: 'var(--text-primary)' }}>连接步道传感器</h3>
        <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>请依次为4个步道垫子选择对应的串口（COM口），每个垫子对应一个端口</p>
        <div className="flex flex-col gap-3 mb-5">
          {[0, 1, 2, 3].map(idx => {
            const status = statuses[idx];
            const error = errors[idx];
            const color = PAD_COLORS[idx];
            return (
              <div key={idx} className="flex items-center gap-3 px-4 py-3 rounded-lg"
                style={{ border: `1px solid ${status === 'connected' ? color : 'var(--border-medium)'}`, background: status === 'connected' ? `${color}08` : 'var(--bg-tertiary)' }}>
                <div className="w-2.5 h-2.5 rounded-full shrink-0"
                  style={{ background: status === 'connected' ? color : status === 'connecting' ? '#F59E0B' : status === 'error' ? '#DC2626' : 'var(--border-medium)' }} />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{PAD_LABELS[idx]}</div>
                ...
                {status !== 'connected' && status !== 'connecting' && (
                  <button onClick={() => connectPad(idx)} className="shrink-0 text-xs font-medium px-3 py-1.5 rounded-md transition-colors" style={{ color: 'white', background: color }}>
                    选择端口
                  </button>
                )}
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleConfirm} disabled={!anyConnected}
            className="zeiss-btn-primary flex-1 py-2.5 text-sm" style={{ opacity: anyConnected ? 1 : 0.5 }}>
            开始评估
          </button>
        </div>
      </div>
...
/* ─── 步态报告组件（使用真实数据） ─── */
function GaitReportContent({ patientInfo, pythonResult: externalResult }) {
...
export default function GaitAssessment() {
  ...
  /* 串口帧录制缓冲区（4个传感器各自独立） */
  const serialFramesRef = useRef({ sensor1: [], sensor2: [], sensor3: [], sensor4: [] });
  ...
  const [csvFiles, setCsvFiles] = useState([null, null, null, null]); // 1.csv ~ 4.csv
  ...
  useEffect(() => {
    SENSOR_KEYS.forEach((key) => {
      footpadServices[key].setOnData((matrix) => {
        ...
        // 录制中收集帧数据（转置回原始格式，与 Python 采集脚本 get_data_from_com_4_part.py 保存的数据一致）
        if (phaseRef.current === 'recording') {
          const original = transposeMatrix(matrix);
          const flat = original.flat();
          serialFramesRef.current[key].push({
            matrix: original,
            flat,
            timestamp: Date.now(),
            max: Math.max(...flat),
            nonZeroCount: flat.filter(v => v > 0).length,
            totalPressure: flat.reduce((a, b) => a + b, 0),
          });
        }
      });
  ...
  /* 对话框连接成功回调 */
  const handleDeviceConnected = useCallback((statuses) => {
    // 更新 connectionState
    const newState = {};
    SENSOR_KEYS.forEach((key, idx) => {
      if (statuses[idx] === 'connected') newState[key] = true;
    });
  ...
  const handleCsvAnalyze = async () => {
    const allReady = csvFiles.every(f => f !== null);
    if (!allReady) return;
    setCsvAnalyzing(true);
    setCsvError('');
    try {
      const csvContents = await Promise.all(csvFiles.map(f => f.text()));
      const res = await analyzeGaitCSV(csvContents);
      if (res.success) {
        ...
        completeAssessment('gait', { completed: true }, { pythonResult: res.data });
      } else {
  ...
  /** 转置矩阵（反转 FootpadSerialService 的 .T，恢复为 Python 采集脚本保存的原始数据格式） */
  ...
  const start = () => {
    ...
    // 清空帧缓冲区
    serialFramesRef.current = { sensor1: [], sensor2: [], sensor3: [], sensor4: [] };
  ...
  const stop = () => {
    ...
    const frames = serialFramesRef.current;
    const hasFrames = SENSOR_KEYS.some(key => frames[key].length > 0);
    ...
    if (hasFrames) {
      ...
      (async () => {
        try {
          const csvContents = SENSOR_KEYS.map(key => framesToCSV(frames[key]));
          const res = await analyzeGaitCSV(csvContents);
          if (res.success) {
            setPythonResult(res.data);
            completeAssessment('gait', { completed: true }, { pythonResult: res.data });
  ...
  const viewReport = () => {
    ...
    completeAssessment('gait', { completed: true }, { sensorData, pythonResult });
  };
  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg" style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-light)' }}>
            {SENSOR_KEYS.map((key, idx) => (
              <div key={key} className="flex items-center gap-1 px-1 py-0.5">
                <div className="w-1.5 h-1.5 rounded-full" style={{ background: connectionState[key] ? PAD_COLORS[idx] : '#d1d5db' }} />
                <span className="text-[10px]" style={{ color: connectionState[key] ? PAD_COLORS[idx] : 'var(--text-muted)' }}>{idx + 1}</span>
              </div>
            ))}
            <span style={{ color: 'var(--border-medium)', margin: '0 1px' }}>|</span>
            {!simRef.current && !SENSOR_KEYS.every(k => connectionState[k]) && (
              <button onClick={handleConnect} className="text-[10px] font-medium" style={{ color: 'var(--zeiss-blue)' }}>连接</button>
            )}
            {!simRef.current && !SENSOR_KEYS.some(k => connectionState[k]) && (
              <>
                <span style={{ color: 'var(--border-medium)', margin: '0 1px' }}>|</span>
            ...
            {(simRef.current || SENSOR_KEYS.some(k => connectionState[k])) && (
              <button onClick={simRef.current ? stopSimulate : handleDisconnect} className="text-[10px] font-medium" style={{ color: C.red }}>断开</button>
            )}
          </div>
          <span className="text-sm font-semibold hidden md:inline" style={{ color: 'var(--text-primary)' }}>{patientInfo?.name || '---'}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center zeiss-overlay animate-fadeIn">
          <div className="zeiss-dialog p-8 flex flex-col items-center gap-4 min-w-[340px] animate-slideUp">
            ...
            <div className="flex gap-3 w-full mt-2">
              <button onClick={() => { setShowComplete(false); completeAssessment('gait', { completed: true }, { sensorData, pythonResult }); navigate('/dashboard'); }}
      ...
      {showCsvDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center zeiss-overlay animate-fadeIn">
          <div className="zeiss-dialog p-6 min-w-[440px] animate-scaleIn">
            <h3 className="text-lg font-bold mb-1" style={{ color: 'var(--text-primary)' }}>导入步态CSV数据分析</h3>
            <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>请导入4个步道传感器的CSV数据文件 (1.csv ~ 4.csv)</p>

            <div className="space-y-3 mb-4">
              {[0, 1, 2, 3].map(idx => (
                <div key={idx}>
                  <label className="text-xs font-medium mb-1.5 flex items-center gap-2" style={{ color: 'var(--text-secondary)' }}>
                    <span className="w-2 h-2 rounded-full" style={{ background: ['#3b82f6', '#22c55e', '#a855f7', '#f97316'][idx] }} />
                    传感器 {idx + 1} ({idx + 1}.csv)
                  </label>
                  <label className="flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer transition-colors"
                    style={{ border: `1px solid ${csvFiles[idx] ? 'var(--zeiss-blue)' : 'var(--border-medium)'}`, background: csvFiles[idx] ? 'var(--zeiss-blue-light)' : 'var(--bg-tertiary)' }}>
                    <div className="flex items-center gap-2 min-w-0">
                      <svg className="w-4 h-4 shrink-0" style={{ color: csvFiles[idx] ? 'var(--zeiss-blue)' : 'var(--text-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                      <span className="text-xs truncate" style={{ color: csvFiles[idx] ? 'var(--zeiss-blue)' : 'var(--text-muted)' }}>
                        {csvFiles[idx] ? csvFiles[idx].name : `选择 ${idx + 1}.csv`}
                      </span>
                    </div>
                    {csvFiles[idx] && (
                      <button onClick={(e) => { e.preventDefault(); setCsvFiles(prev => { const n = [...prev]; n[idx] = null; return n; }); }} className="shrink-0 ml-2 w-5 h-5 flex items-center justify-center rounded-full" style={{ background: 'rgba(0,0,0,0.1)' }}>
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                    )}
                    <input type="file" accept=".csv" className="hidden" onChange={e => { const f = e.target.files?.[0] || null; setCsvFiles(prev => { const n = [...prev]; n[idx] = f; return n; }); }} />
                  </label>
                </div>
              ))}
            </div>
            ...
            <div className="mb-4 flex gap-2">
              <label className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-xs font-medium transition-colors"
                style={{ border: '1px dashed var(--border-medium)', color: 'var(--text-muted)', background: 'var(--bg-tertiary)' }}>
                ...
                <input type="file" accept=".csv" multiple className="hidden" onChange={e => {
                  ...
                  const newFiles = [null, null, null, null];
                  files.forEach(f => {
                    const match = f.name.match(/^(\d)\.csv$/);
                    if (match) {
                      const idx = parseInt(match[1]) - 1;
                      if (idx >= 0 && idx < 4) newFiles[idx] = f;
                    }
                  });
                  const unmatched = files.filter(f => !f.name.match(/^[1-4]\.csv$/));
                  let unmatchedIdx = 0;
                  for (let i = 0; i < 4 && unmatchedIdx < unmatched.length; i++) {
                    if (!newFiles[i]) { newFiles[i] = unmatched[unmatchedIdx++]; }
                  }
                  setCsvFiles(newFiles);
                }} />
              </label>
              <label className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-xs font-medium transition-colors"
                style={{ border: '1px dashed var(--zeiss-blue)', color: 'var(--zeiss-blue)', background: 'var(--zeiss-blue-light)' }}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
                导入文件夹
                <input type="file" className="hidden" webkitdirectory="" directory="" onChange={e => {
                  const files = Array.from(e.target.files || []);
                  const newFiles = [null, null, null, null];
                  // 从文件夹中匹配 1.csv ~ 4.csv
                  files.forEach(f => {
                  ...
                  const matched = newFiles.filter(Boolean).length;
                  if (matched === 0) {
                    setCsvError('文件夹中未找到 1.csv ~ 4.csv，请确认文件命名');
                  } else {
                    setCsvError(matched < 4 ? `已识别 ${matched}/4 个文件，缺少: ${[1,2,3,4].filter((_, i) => !newFiles[i]).map(n => `${n}.csv`).join(', ')}` : '');
                    setCsvFiles(newFiles);
                  }
                }} />
              </label>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleCsvAnalyze} disabled={!csvFiles.every(f => f) || csvAnalyzing}
                className="zeiss-btn-primary flex-1 py-2.5 text-sm flex items-center justify-center gap-2"
                style={{ opacity: (!csvFiles.every(f => f) || csvAnalyzing) ? 0.5 : 1 }}>
            ...
            {phase === 'processing' && (
              <div className="absolute inset-0 flex flex-col items-center justify-center zeiss-overlay rounded-xl">
                ...
                <p className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
                  {serialAnalyzing ? '正在将串口数据发送至后端分析，请稍候...' : '正在分析步态数据，请稍候...'}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="absolute bottom-6 z-20 flex flex-col items-center gap-3">
              {phase === 'idle' && deviceStatus === 'connected' && (
                <>
                  ...
                  <button onClick={() => { setCsvFiles([null, null, null, null]); setCsvError(''); setShowCsvDialog(true); }}
              ...
              {phase === 'idle' && deviceStatus !== 'connected' && (
                <div className="flex items-center gap-3 px-5 py-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(8px)', border: '1px solid var(--border-light)', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                  <span className="text-xs" style={{ color: 'var(--text-muted)' }}>请先连接设备或选择模拟模式</span>
                  <button onClick={handleConnect} className="zeiss-btn-primary text-[11px] py-1.5 px-4">连接设备</button>
                  <button onClick={handleSimulate} className="text-[11px] py-1.5 px-4 rounded-md font-medium" style={{ background: 'var(--success-light)', color: 'var(--success)', border: '1px solid var(--success)' }}>模拟模式</button>
                </div>
              )}
              {phase === 'recording' && (
                <>
