...
import { analyzeStandingCSV } from '../../lib/gripPythonApi';

/* ─── 单个足垫串口服务实例 ─── */
const footpadService = new SingleFootpadSerialService('standing', '站立足垫');
...
export default function StandingAssessment() {
  ...
  const handleSerialData = useCallback((matrix) => {
    ...
    // 采集中保存帧数据（转置回原始格式，与 Python 采集脚本保存的数据一致）
    if (isRecordingRef.current) {
      if (currentRawFlat.current) {
        // 模拟模式：使用原始flat数据
        collectedFrames.current.push(currentRawFlat.current);
      } else {
        // 真实硬件模式：FootpadSerialService 做了转置，需要转置回去
        const original = transposeMatrix(matrix);
        collectedFrames.current.push(original.flat());
      }
    }
  ...
  const handleConnect = useCallback(async () => {
    ...
    try {
      footpadService.setOnData(handleSerialData);
      footpadService.setOnLog((msg, type) => {
        console.log(`[Footpad ${type}] ${msg}`);
      });
      footpadService.setOnConnectionChange((connected) => {
  ...
  const handleDisconnect = useCallback(async () => {
    // 停止模拟定时器
    stopSimulation();
    await footpadService.disconnect();
    setDeviceStatus('disconnected');
  ...
  // ─── 滤波阈值（FootpadSerialService 通过 configure 设置） ───
  useEffect(() => {
    footpadService.configure({ noiseThreshold: filterThreshold });
  }, [filterThreshold]);
  ...
  /** 将采集的帧数据转为 CSV 文本（与 Python load_csv_data 兼容） */
  const framesToCSV = useCallback((frames) => {
    const lines = ['idx,data'];
    for (let i = 0; i < frames.length; i++) {
      const flat = frames[i]; // 已经是 flat 数组
      lines.push(`${i},"[${flat.join(',')}]"`);
    }
    return lines.join('\n');
  }, []);
  ...
  const stopRecording = async () => {
    ...
    try {
      // 生成 CSV 并调用 Python 后端分析
      const csvContent = framesToCSV(frames);
      console.log(`[站立评估] 采集 ${frames.length} 帧，调用 Python 分析...`);
      const res = await analyzeStandingCSV(csvContent);
      if (res.success) {
        setPythonResult(res.data);
        setPythonImages(res.images || null);
        // 同时生成本地报告作为备用
        const report = generateFootReport(frames);
        setReportData(report);
        setShowCompleteDialog(true);
      } else {
        console.error('Python 分析失败，使用本地报告');
        const report = generateFootReport(frames);
        setReportData(report);
        setShowCompleteDialog(true);
      }
    } catch (err) {
      console.error('Python 后端调用失败，使用本地报告:', err);
      const report = generateFootReport(frames);
      setReportData(report);
      setShowCompleteDialog(true);
    }
  ...
  const handleCsvAnalyze = async () => {
    ...
    try {
      ...
      const res = await analyzeStandingCSV(text);
      if (res.success) {
        setPythonResult(res.data);
        setPythonImages(res.images || null);
        ...
        completeAssessment('standing', { completed: true }, { pythonResult: res.data });
      } else {
  ...
  useEffect(() => {
    return () => {
      ...
      if (simIntervalRef.current) clearInterval(simIntervalRef.current);
      // 组件卸载时断开足垫连接
      if (footpadService.getIsConnected()) {
        footpadService.disconnect().catch(() => {});
      }
    };
  }, []);
      ...
      <div className="h-screen w-screen flex flex-col overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
        ...
        <main className="flex-1 min-h-0 overflow-auto">
          {reportMode === 'dynamic' ? (
            ...
            <StandingReport reportData={reportData} patientInfo={patientInfo} pythonResult={pythonResult} pythonImages={pythonImages} />
          )}
        </main>
      </div>
    );
      ...
      {showCsvDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center zeiss-overlay animate-fadeIn">
          <div className="zeiss-dialog p-6 min-w-[400px] animate-scaleIn">
            ...
            <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>请导入足底压力传感器的CSV数据文件</p>
            ...
            <div className="mb-4">
              ...
              <label className="flex items-center justify-between px-3 py-2.5 rounded-lg cursor-pointer transition-colors"
                style={{ border: `1px solid ${csvFile ? 'var(--zeiss-blue)' : 'var(--border-medium)'}`, background: csvFile ? 'var(--zeiss-blue-light)' : 'var(--bg-tertiary)' }}>
                <div className="flex items-center gap-2 min-w-0">
                  <svg className="w-4 h-4 shrink-0" style={{ color: csvFile ? 'var(--zeiss-blue)' : 'var(--text-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  <span className="text-sm truncate" style={{ color: csvFile ? 'var(--zeiss-blue)' : 'var(--text-muted)' }}>
                    {csvFile ? csvFile.name : '选择CSV文件'}
                  </span>
                </div>
                {csvFile && (
                  <button onClick={(e) => { e.preventDefault(); setCsvFile(null); }} className="shrink-0 ml-2 w-5 h-5 flex items-center justify-center rounded-full" style={{ background: 'rgba(0,0,0,0.1)' }}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleCsvAnalyze} disabled={!csvFile || csvAnalyzing}
                className="zeiss-btn-primary flex-1 py-2.5 text-sm flex items-center justify-center gap-2"
                style={{ opacity: (!csvFile || csvAnalyzing) ? 0.5 : 1 }}>
            ...
            <div className="absolute bottom-10 z-20 flex flex-col items-center gap-3">
              {phase === 'idle' && deviceStatus === 'connected' && (
                <>
                  ...
                  <button onClick={() => { setShowCsvDialog(true); setCsvError(''); setCsvFile(null); }}
