...
/** 将 peakMatrix (64x64 2D) 转为 flat 4096 数组 */
function flattenPeakMatrix(peakMatrix) {
  if (!peakMatrix) return null;
  if (peakMatrix.length !== 64) return null;
  const flat = new Array(4096);
  for (let r = 0; r < 64; r++) {
    for (let c = 0; c < 64; c++) {
      flat[r * 64 + c] = (peakMatrix[r] && peakMatrix[r][c]) || 0;
    }
  }
  return flat;
}
...
function drawFootCOP(ctx, copPts, peakData, colStart, colEnd, offsetX, width, hoveredIdx, title, sectionCoords) {
  ...
  // 计算足印边界 — 优先使用 sectionCoords（与足弓图一致）
  let bounds;
...
export default function InteractiveCOPChart({ leftCopRaw, rightCopRaw, peakFrameData, leftSectionCoords, rightSectionCoords, peakMatrix }) {
  ...
  // 兼容：如果传了 peakMatrix (2D) 但没传 peakFrameData (flat)，自动转换
  const peakFlat = peakFrameData || flattenPeakMatrix(peakMatrix);

  // sectionCoords 归一化
  const leftSC = leftSectionCoords ? normalizeSectionCoords(leftSectionCoords) : null;
  const rightSC = rightSectionCoords ? normalizeSectionCoords(rightSectionCoords) : null;
  ...
  const draw = useCallback(() => {
    ...
    drawFootCOP(ctx, leftPanelCop, peakFlat, 0, 32, 0, halfW,
      hoveredPoint?.side === 'left' ? hoveredPoint.idx : null, '左脚 COP', leftSC);
    drawFootCOP(ctx, rightPanelCop, peakFlat, 32, 64, halfW, halfW,
  ...
  }, [leftPanelCop, rightPanelCop, hoveredPoint, peakFlat, leftSC, rightSC]);
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (sc && sc.flat().length > 0) {
      ...
      bounds = { minR, maxR, minC, maxC };
    } else if (peakFlat && peakFlat.length === 4096) {
      bounds = getFootBounds(peakFlat, colStart, colEnd, pts);
    } else {
      bounds = { minR: 0, maxR: 63, minC: colStart, maxC: colEnd - 1 };
    }
    const rowSpan = bounds.maxR - bounds.minR || 1;
  ...
  }, [leftPanelCop, rightPanelCop, peakFlat, leftSC, rightSC]);
