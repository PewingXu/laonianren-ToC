...
export function useSitStandReport({ gateway, recordId, mapper }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const assessment = record?.assessments?.sitstand;
        if (
          record === null
        ...
        const report = await gateway.getAssessmentReport('sitstand', {
          recordId,
